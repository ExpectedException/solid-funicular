...
class account:
    def __init__(self, id, steam, email, password, isbanned):
        self.id = id
        self.steam = steam
        self.email = email
...
with open('data_file.json', 'r') as json_file:
    acc_data = json.loads(json_file.read())
    for u in acc_data:
        accounts_list.append(account(**u))
    newstr = str(accounts_list)
    num = newstr.count(' ') + 1
...
def main():
    ...
    profile = FirefoxProfile("C:\\Users\\PussyDestroyer\\AppData\\Roaming\\Mozilla\\Firefox\\Profiles\\enqlkevv.Default")
    ...
    wait.until(ec.element_to_be_clickable((By.LINK_TEXT, '2019'))).click()
